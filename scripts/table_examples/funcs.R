...
efficacy_models <- function(data, var, wk) {
  # Subset to analyze
  week <- data %>%
    filter(AVISITCD == as.character(glue('Wk{wk}')))

  # Create an ordered factor variable for the models
  week['TRTPCD_F'] <- factor(week$TRTPCD, levels=c('Xan_Hi', 'Xan_Lo', 'Pbo'))
  ...
  # Basic linear model
  if (var == "CHG") {
    model1 <- lm(CHG ~ TRTDOSE + SITEGRP + BASE, data=week)
    model2 <- lm(CHG ~ TRTPCD_F + SITEGRP + BASE, data=week)
  } else {
    model1 <- lm(VAL ~ TRTDOSE + SITEGRP, data=week)
    model2 <- lm(VAL ~ TRTPCD_F + SITEGRP, data=week)
  }
  ...
  return(bind_rows(dose_response, pw_final))
