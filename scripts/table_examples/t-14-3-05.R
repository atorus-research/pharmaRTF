...
summary_portion <- bind_rows(summary_data(VAL, 'BL', 'Baseline'),
                             summary_data(VAL, 'Wk16', 'Week 16'),
                             summary_data(CHG, 'Wk16', 'Change from Baseline')) %>%
  pad_row()
...
# Basic linear model
model <- lm(CHG ~ TRTDOSE + SITEGRP + BASE, data=week16)
...
week16['TRTPCD_F'] <- factor(week16$TRTPCD, levels=c('Xan_Hi', 'Xan_Lo', 'Pbo'))

# Linear model but use treatment group as a factor now
model <- lm(CHG ~ TRTPCD_F + SITEGRP + BASE, data=week16)
