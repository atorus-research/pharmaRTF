...
spec_df <- read.csv("~/pharmaRTF/vignettes/specs.csv")
...
for(i in seq(ceiling(ncol(final)/max_cols_to_display))) {
  ...
  cat(kable(final[,seq(start, end)], "latex", booktabs = TRUE, longtable = TRUE))
}
...
# Audit information
```{r log_info}
kable(readRDS("~/pharmaRTF/vignettes/Validation/vur_auto.Rds")[, c("CheckID", "OutputFile", "Response", "Log")],
