...
test_cases <- read.csv("~/pharmaRTF/vignettes/test_cases.csv")
...
trace_matrix <- function(specs) {
  ## Change to ; later
  specs_tested <- unlist(str_split(test_cases$Specs, ","))
  daply(specs, 1, function(x) {
    spec_i <- as.character(x)
    data.frame(
      spec_i = ifelse(spec_i %in% specs_tested, "X", "")
    )
  })
}
