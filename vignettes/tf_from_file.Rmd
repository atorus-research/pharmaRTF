...
(For more information about the special text fields you see here, see our [advanced usage](advanced_usage.html) vignette)
...
With this function now available, we can see the data frame that the function processes and returns. 

```{r use_custom_Reader}
example_custom_reader('../data/titles.xlsx', table_number = '14-2.01')
```

In this example, the data processing was very simple - but in a more advanced case this function could read from multiple sheets, merge in default titles and footnotes, read from different data sources - whatever you suits your process.

## Putting It Together

Now with an understanding of how a custom reader function works, let's tie it all together. Let's assume that our table has been programmed, and that the huxtable table is ready in our session (see the [huxtable tips vignette](huxtable_tips.html) to for tips on how to prepare your table). 

_Note: The data here comes from the CDISC Pilot, which you can read about [here](https://www.cdisc.org/sdtmadam-pilot-project). Also, see our replication of the tables from this CDISC pilot in our [Github repository](link_tbd.com)._

```{r prep_huxtable, echo=FALSE, results='hide'}
huxtable::as_hux(load('../data/t14_4_01.Rdata'))
```
...
Here's how you would go about creating your `rtf_doc` object. (For more information about the functions being used here, see our [advanced usage](advanced_usage.html) vignette)
...
Within the `titles_and_footnotes_from_df` function you can see the parmaters used:
